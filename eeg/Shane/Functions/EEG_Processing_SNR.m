...
path_data = hera('Projects/7TBrainMech/scripts/eeg/Shane/SNR/ICAwhole/');
CleanICApath = hera('Projects/7TBrainMech/scripts/eeg/Shane/SNR/AfterWhole/ICAwholeClean/');

EEGfileNames = dir([path_data '/*.set']);
for fidx = 1:length(EEGfileNames)
    filename = EEGfileNames(fidx).name;
    
    locs = file_locs_SNR(fullfile(path_data,filename));
...
path_data = hera('Projects/7TBrainMech/scripts/eeg/Shane/SNR/AfterWhole/ICAwholeClean/');
epoch_folder =  hera('Projects/7TBrainMech/scripts/eeg/Shane/SNR/AfterWhole/epoch/');
epoch_rj_marked_folder = hera('Projects/7TBrainMech/scripts/eeg/Shane/SNR/AfterWhole/epochclean/');
...
revisar = {};
for currentEEG = 267:size(EEGfileNames,1)
...
revisar{currentEEG} = epochlean_SNR(inputfile,epoch_folder,epoch_rj_marked_folder);
end
...
%% files
folder = hera('Projects/7TBrainMech/scripts/eeg/Shane/SNR/AfterWhole/epochclean/');
...
%% Homogenize Chanloc
datapath = hera('Projects/7TBrainMech/scripts/eeg/Shane/AudSteadyState/SNR/ICAwholeClean');
outpath = hera('Projects/7TBrainMech/scripts/eeg/Shane/AudSteadyState/SNR/ICAwholeClean_homogenize');
...
for currentEEG = 1:length(setfiles)
    ...
    inputfile = [path_data, filename];
    homogenizeChanLoc_SNR(inputfile,correction_cap_location,outpath,filename)
end
