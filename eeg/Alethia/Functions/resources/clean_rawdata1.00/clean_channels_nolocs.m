function signal = clean_channels_nolocs(signal, min_corr, ignored_quantile, clchan_window_len, max_broken_time, linenoise_aware)
...
%                     0.1.
%
%   CleanChannelWindowLength : Length of the windows (in seconds) for which correlation is computed; ideally
...
%
% History
% 03/26/2019 Makoto. The function name fixed!!!
%
...
if ~exist('clchan_window_len','var') || isempty(clchan_window_len) clchan_window_len = 2; end
...
clchan_window_len = clchan_window_len*signal.srate;
wnd = 0:clchan_window_len-1;
offsets = 1:clchan_window_len:S-clchan_window_len;
W = length(offsets);
...
% ignored samples
removed_channels = sum(flagged,2)*clchan_window_len > max_broken_time;
