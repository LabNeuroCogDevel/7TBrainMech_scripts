function [data] = w2_erps_2_conditions_by_channel_EEG(condition_1,condition_2,cycles,freq_range,alpha,fdr,scale,basenorm,erps_max,path_to_save,EEG,data)
%-------DESCRIPTION---------------
...
path_to_files = fullfile(data.parent_directory, path_to_files); %toma path_to_files y path_to_save como paths relativos
...
c2_erps=[];
c1_c2_erps=[];
tfX = []; %5D matrix single trial results (channels,freq,times,epochs,subjects)
c2_tfX = [];
c1_erpsboot = []; %4D matrix where statistical results will be stored (channels, freq, times, subjects)
c2_erpsboot = [];
c1_c2_erpsboot = [];
mbases = []; %2D matrix, where baseline powers are stored (base,subjects)
c2_mbases = [];
c1_data = [];
c2_data = [];
...
for suj = 1 : file_nr
    ...
    %condition 1 EEG
    c1_EEG = pop_selectevent( EEG, 'type', {condition_1} ,'deleteevents','off','deleteepochs','on','invertepochs','off'); %channels*time*epochs cond_1
    %condition 2 EEG
   
    c2_EEG = pop_selectevent( EEG, 'type', {condition_2} ,'deleteevents','off','deleteepochs','on','invertepochs','off'); %channels*time*epochs cond_2
    ...
    for ch = 1 : ch_nr        
        ...
        c1_tmpsig = reshape( c1_tmpsig, length(ch), size(c1_tmpsig,2)*size(c1_tmpsig,3)); %¿Por qué hace esto? length(ch) =1 porque ch es un escalar. ¿Debería ser ch_nr, en luhar de length(ch). Pruebo eso.
        %c1_tmpsig = reshape( c1_tmpsig, ch_nr, size(c1_tmpsig,2)*size(c1_tmpsig,3));
        ...
        %generated)
        %plot
        [P,R,mbase,timesout,frequencies,Pboot,Rboot,alltfX,g] = newtimef_2_conditions( {c1_tmpsig(:, :),c2_tmpsig(:, :)}, length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles, 'plotersp','off', 'plotitc' , 'off','topovec', ch, 'elocs', EEG.chanlocs,'title',{condition_1 condition_2},'freqs',freq_range,'alpha',alpha,'mcorrect',fdr,'scale',scale,'basenorm',basenorm,'erspmax',erps_max, 'ntimesout', 400, 'padratio', 4,'baseline',[0],'caption',chanlabel) ;                        
        
        %[P,R,mbase,timesout,frequencies,Pboot,Rboot,alltfX,g] = m_newtimef( {c1_tmpsig(:, :),c2_tmpsig(:, :)}, length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles, 'plotersp','off', 'plotitc' , 'off','topovec', ch, 'elocs', EEG.chanlocs,'title',{condition_1 condition_2},'freqs',freq_range,'alpha',alpha,'mcorrect',fdr,'scale',scale,'basenorm',basenorm,'erspmax',erps_max, 'ntimesout', 400, 'padratio', 4,'baseline',[0],'caption',chanlabel) ;                        
        
        %[P,R,mbase,timesout,frequencies,Pboot,Rboot,alltfX,g] = m_newtimef( c1_tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles, 'plotersp','off', 'plotitc' , 'off','topovec', ch, 'elocs', EEG.chanlocs,'title',condition_1,'freqs',freq_range,'alpha',alpha,'mcorrect',fdr,'scale',scale,'basenorm',basenorm,'erspmax',erps_max, 'ntimesout', 400, 'padratio', 4,'baseline',[0],'caption',chanlabel) ;                        
        
        %alltfX tiene una dimensión más (freq*tiempo*trials) que no tienen
        %P,R (freq*tiempo).
        %load results in matrices of overall results    
        c1_erps(ch,:,:,suj) = P{1}; %P=[P1,P2,P1-P2], cada uno tiene dimensiones de frec*tiempo (uno para cada canal, para cada sujeto)
        
        %erps(ch,:,:) = P{1}; %P=[P1,P2,P1-P2], cada uno tiene dimensiones de frec*tiempo (uno para cada canal, para cada sujeto)
        ...
        c1_mbases(ch,:,suj) = mbase{1}; %mbase=[mbase1, mbase2], cada uno es un array cuya cantidad de elementos es igual a la cantidad de frecuencias analizadas.        
                                        %=> c1_mbases tiene dimensiones
                                        %chan*freq*suj

        %mbases(ch,:) = mbase; %mbase=[mbase1, mbase2], cada uno es un array cuya cantidad de elementos es igual a la cantidad de frecuencias analizadas.        
                                
        c2_mbases(ch,:,suj) = mbase{2};                        
    end
    
    s_tfX = {c1_tfX,c2_tfX};
    s_erps = {c1_erps c2_erps c1_c2_erps};
    if ~isnan(alpha)
        s_erpsboot = {c1_erpsboot c2_erpsboot c1_c2_erpsboot};
    end
    ...
    save(mat_name, 's_tfX', 's_erps', 's_erpsboot', 'timesout','frequencies','g','-v7.3'); %guarda los datos en el archivo Bil_EstX_condition_1_condition_2.mat (sujeto a sujeto)

    %save(mat_name, 'erps', 'mbases', 'timesout','frequencies','g','-v7.3'); %guarda los datos en el archivo Bil_EstX_condition_1_condition_2.mat (sujeto a sujeto)
...
mbases = {c1_mbases,c2_mbases (c1_mbase + c2_mbase)/2};


%save results in mat
prefix_file_name_to_save = [condition_1 '_' condition_2]; %condition1_condition2
mat_name = fullfile(path_to_save,[prefix_file_name_to_save '.mat']);
save(mat_name, 'erps','erpsboot','mbases','timesout','frequencies','g','-v7.3');
clear erps  erpsboot mbases c1_erpsboot c2_erpsboot c2_c2_erpsboot c1_mbases c2_mbases
