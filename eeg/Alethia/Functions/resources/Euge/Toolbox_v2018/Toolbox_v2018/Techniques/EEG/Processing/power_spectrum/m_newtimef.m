...
if ~isnan(g.alpha)
    if isempty(find(~isnan(g.pboot))) % if ERSP lims not provided
        ...
        exactp_ersp = compute_pvals(P, Pboottrials); %acá ya se hace corrección por múltiples comparaciones
        if strcmpi(g.mcorrect, 'fdr')
            ...
            maskersp = exactp_ersp <= g.alpha; %acá se queda con los píxeles representativos, solamente.
        end;
    end;    
