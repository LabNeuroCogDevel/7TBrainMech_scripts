%% preprocess EEG .bdf (biosemi) files using EEGLAB, the PREP pipeline, and ICA
%NDT May 2018 for Peter Bachman
%this script requires the EEGLAB toolbox for MATLAB, along with the
%PREP plugin and the SASICA plugin
%note: this script will only process .bdf files for which a final
%processing result doesn't exist in the output folder

%Variables below marked with "%<<<CHANGE" need to be specified for your
%computer/setup and desired processing steps
 RawDataLocation = 'E:\LunaEEG\raw data\PreprocessPipeline\128-channel array'; %<<<CHANGE %need to use '\' not '/'
 ProcessedDataLocation = 'E:\LunaEEG\raw data\PreprocessPipeline\128-channel array\Output\'; %<<<CHANGE %need to use '\' not '/'
 eeglabpath='C:/Users/theisn/Documents/MATLAB/eeglab14_1_1b'; %<<<CHANGE %need to use '/' not '\'
 channelMode=128; %<<<CHANGE either 64 or 128
 %specify which steps to run
 runPREP = true; %set to true to run prep, else use false
 runICA = true; %set to true to run Independant Component Analysis, else use false

 %% Compute!
 Append2output='PREP_HighPass_ICA.set'; %this is how the final file will be named
 Append3output='PREP_HighPass_ICA_SAS.set'; %this is how the final file will be named
 files=dir(RawDataLocation); %create list of RawDataLocation contents
 EEGfileNames={files.name}; %get the filename values from the above list
 addpath(eeglabpath); %add EEGLAB to the MATLAB path
for eeg=1:length(EEGfileNames) %begin loop through file names
    %check if the file/folder is a .bdf (biosemi) file
     EEGfileName=EEGfileNames{eeg}; %capture the Name as single string variable from Names array
     basicOutputFileName=EEGfileName(1:end-4); %remove the '.bdf' string
     OutputFileName=sprintf('%s_%s',basicOutputFileName,Append2output); %write a basic output name string
     SASICAOutputFileName=sprintf('%s_%s',basicOutputFileName,Append3output); %write a basic output name string
     OutputFile=sprintf('%s%s',ProcessedDataLocation,OutputFileName); %write string for the full output directory and file
     if exist(OutputFile, 'file') == 0 %if the output file doesn't exist then start processing
    
        if (any(regexp(EEGfileNames{eeg},'bdf$'))==1) %see if the filename ends ($) in 'bdf'
...
%% Load .bdf file
         ...
         startSubMessage=sprintf('preprocessEEGdata: About to import %s for processing', EEGfileName);
         disp(startSubMessage);
         EEGfile=sprintf('%s%s',RawDataLocation,EEGfileName); %write string for full input directory and fule
         ...
         EEG.setname=EEGfileName; %name the EEGLAB set (this is not the set file itself)
...
%% Else load the .set file.  NOTE: User must create this set file from a bdf file
        elseif(any(regexp(EEGfileNames{eeg},'set$'))==1)  %%load up the .set files instead  
...
%% Run PREP
 ...
 %% run ICA
     ...
     end %end of IF output doesn't exist 

   close all %close eeglab
   clearvars -except runICA runPREP EEGfileNames eeg RawDataLocation ProcessedDataLocation Append2output Append3output eeglabpath files channelMode;%not variables that we still need, i.e. whatever is being looping on %reset workspace
 end  %end FOR files in raw directory
 disp('Finished');
